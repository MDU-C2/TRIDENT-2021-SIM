...
SetpointsSelector::SetpointsSelector(::ros::NodeHandle& nh)
  : priority_list_({{"open_loop", 0.0}, {"safety", 0.0}})
{
  pub_ts_ = nh.advertise<cola2_msgs::Setpoints>(cola2::ros::getNamespace() + "/setpoints_selector/thruster_setpoints", 10, true);
}

bool
SetpointsSelector::acceptSetpoints(const cola2_msgs::Setpoints& msg)
{
